...
public class RandomDateOne extends RandomElement {
    public RandomDateOne() {
        super("Random Date One (YYYY.MM.DD)");
        putSettings(RandomizersCache.SETTING_ANDROID_BUILD_DATE_ONE);
    }

    @Override
    public void randomize(RandomizerSessionContext context) {
        for (String setting : getSettings()) {
            context.pushValue(setting, RanAndUtils.dateOne(context), false);
        }
    }
}