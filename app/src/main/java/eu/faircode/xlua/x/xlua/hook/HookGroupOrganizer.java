...
public class HookGroupOrganizer {
    ...
    public List<XHook> getHooksForSettings(List<String> setting_names) {
        ...
        for(Map.Entry<String, HookGroup> entry : groups.entrySet()) {
            ...
            for(XHook hook : group.hooks) {
                ...
                List<String> hook_settings = hook.settings;
                boolean found = false;
                if(ListUtil.isValid(hook_settings)) {
                    for(String hook_setting : hook_settings) {
                        ...
                        if(lower_names.contains(lowered)) {
                            hooks.add(hook);
                            if(DebugUtil.isDebug())
                                Log.d(TAG, Str.fm("Found Possible Hook to Setting by Settings! Setting Count=%s Hook Name=%s Hook ID=%s  UID=%s  Package Name=%s", setting_names.size(), hook.name, hook.getObjectId(), "null", "null"));
                ...
                if(!found && !TextUtils.isEmpty(hook.luaScript)) {
                    String script = hook.luaScript.toLowerCase();
                    for(String setting_name : lower_names) {
                        ...
                        if(script.contains(format_1) || script.contains(format_2)) {
                            hooks.add(hook);
                            if(DebugUtil.isDebug())
                                Log.d(TAG, Str.fm("Found Possible Hook to Setting by Lua Script! Setting Name=%s Hook Name=%s Hook ID=%s  UID=%s  Package Name=%s", setting_name, hook.name, hook.getObjectId(), "null", "null"));
    ...
    public void initGroups(List<XHook> hooks, Context context) {
        groups.clear();
        for(XHook hook : hooks) {
            HookGroup group = groups.get(hook.group);
            if(group == null) {
                group = new HookGroup();
                groups.put(hook.group, group);
                ...
                group.title = (group.resourceId > 0 ? resources.getString(group.resourceId) : hook.group);
                group.groupId = GroupHelper.getGroupId(group.name);
                group.hasWarning = HookWarnings.hasWarning(context, group.name);

                if(DebugUtil.isDebug())
                    Log.d(TAG, Str.fm("Created Group for Hook:%s  Group Name=%s UID=%s Package Name=%s", hook.getObjectId(), hook.group, "null", "null"));
            }
    ...
    public void collectApp(AppXpPacket app, List<XHook> hooks, Context context, SharedRegistry viewRegistry) {
    ...
    public static List<String> getHookIdsFromSettingPackets(Context context, List<SettingPacket> settings) {
        ...
        List<XHook> filteredHooks = organizer.getHooksForSettings(allSettings);
        if(DebugUtil.isDebug())
